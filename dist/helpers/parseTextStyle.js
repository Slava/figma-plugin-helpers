...
var _lodash3 = _interopRequireDefault(require("lodash.uniqwith"));
  ...
  textStyle.forEach((style, index) => {
    if (re2.test(style.characters)) {
      ...
      if (ls === null) {
        ...
        // multiple-line text
        style = (0, _lodash2.default)(style);
...
function joinTextLinesStyles(textStyle, addNewlineCharacters = false) {
  const tStyle = (0, _lodash2.default)(textStyle);
  let newline = '';
...
async function applyTextStyleToTextNode(textStyle, textNode, isLoadFonts = true) {
  if (isLoadFonts) {
    ...
    fonts = (0, _lodash3.default)(fonts, _lodash.default);
...
function changeCharactersTextStyle(textStyle, characters) {
  textStyle = (0, _lodash2.default)(textStyle);
  let n = 0;
...
function changeTextStyle(textStyle, styleName, newValue, beforeValue) {
  textStyle = (0, _lodash2.default)(textStyle);
  textStyle.forEach(style => {
    if (beforeValue === undefined || beforeValue !== undefined && (0, _lodash.default)(style[styleName], beforeValue)) {
      ;
...
function isEqualLetterStyle(letter, textStyle) {
  ...
  for (const key in letter) {
    if (key !== 'characters') {
      if (!(0, _lodash.default)(letter[key], textStyle[key])) {
        // property varies
        // stop searching
        is = false;
        break;
